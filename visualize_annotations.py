...
def main(args):
    ...
    scannet_path = os.path.join(current_dir, 'data', 'ScanNetpp')
    annotations_path = os.path.join(scannet_path, 'annotations')
    scans_path = os.path.join(scannet_path, 'data')
    ...
    for scene_name in scene_list:
        ...
        if not os.path.exists(scene_path) or not os.path.exists(annotation_file):
            print('Data not available for scene ' + str(scene_name))
            continue

        scene_mesh = load_scene_mesh(scene_path)
        ...
        for color_id in np.unique(final_inst_seg_3d):
            ...
            color_id_bound = color_id % COLOR_DETECTRON2.shape[0]
            pcl_color_tmp[final_inst_seg_3d == int(color_id)] = COLOR_DETECTRON2[int(color_id_bound), :]
